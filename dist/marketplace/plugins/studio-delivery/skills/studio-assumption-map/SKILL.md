...
2. Gather the lens inputs: brief, constraints, unknowns, and available evidence
3. Apply the lens method: Rank assumptions by uncertainty and impact, assign an owner, and choose the cheapest useful test.
4. Produce the lens output: assumption map ordered by risk with evidence and validation sequence
5. Enforce the stop condition: Stop or mark UNPROVEN when a load-bearing assumption has no owner or test.
6. Follow the escalation path: Escalate high-impact assumptions that cannot be cheaply tested.
