...
name: studio-status
description: Report canonical project state, drift, blockers, proof, and exactly one next valid action.
---
# Studio Status

Use this front-door only when the request needs reconcile .project state, GitHub projections, Drive authorities, and current SHA.

## Contract

- Read current project state, governing requirements, and the current SHA before making a claim.
- Produce or update only the owned outputs: `CURRENT_STATE`.
- This skill does not own silent mutation.
...
- Input: current project state and the request for reconcile .project state, GitHub projections, Drive authorities, and current SHA
- Method: Apply a bounded review of reconcile .project state, GitHub projections, Drive authorities, and current SHA and preserve the evidence trail.
- Output: named CURRENT_STATE outputs with evidence and one next action
