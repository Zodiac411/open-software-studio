...
This account-level ChatGPT Pro view exposes the personal Plugins and Skills directories, but not the workspace-admin marketplace importer. Making the repository public does not add that control; use a supported workspace-admin account for marketplace import, or upload individual skills through `Plugins > Skills > Create` when that surface is offered.
